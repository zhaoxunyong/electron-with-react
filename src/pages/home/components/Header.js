...
class Header extends Component {
    ...
    componentDidMount = () => {
        // TODO: 需要后台从session中取当前用户信息显示
        /* http.get('').then(rs => {
            this.setState({
                loginName: rs.loginName,
                trueName: rs.userName
            })
        }) */
    }
    ...
    render() {
        ...
        return (
            <Fragment>
                <Row>
                    <Col span={1}>
                        <Icon className={HomeStyle.myIcon} type={collapsed ? 'menu-unfold' : 'menu-fold'} onClick={toggle} />
                    </Col>
                    <Col span={2}>
                        <Switch checked={this.props.theme === 'dark'} onChange={this.props.changeTheme} checkedChildren="Dark" unCheckedChildren="Light" />
                    </Col>
                    <Col span={4}>
                        <Breadcrumb />
                    </Col>
                    <Col span={7}>{/* <FormattedMessage id="App.datePicker.title" />
            <DatePicker /> */}</Col>
                    <Col span={5}>
                    ...
                    <Col span={2}>
                        <Dropdown key={1} overlay={menu} onVisibleChange={this.handleMenuVisibleChange} visible={this.state.menuVisible}>
                    ...
                    <Col span={1}>
                        <Dropdown key={2} overlay={lang} onVisibleChange={this.handleLangVisibleChange} visible={this.state.langVisible}>
                            {/* eslint-disable-next-line */}
                            <span className="ant-dropdown-link" style={{ cursor: 'pointer' }}>
                                <Icon type="global" />
                            </span>
